...
static void ImGui_ImplVulkan_CreateDescriptorSetLayout(VkDevice device, const VkAllocationCallbacks *allocator)
{
	...
	std::vector<uint32_t> layoutBindingFlags;
	layoutBindingFlags.emplace_back(VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT);
	layoutBindingFlags.emplace_back(VK_DESCRIPTOR_BINDING_UPDATE_UNUSED_WHILE_PENDING_BIT);
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
	...
	if (!g_DescriptorSetLayout)
	{
		...
		std::vector<uint32_t> layoutBindingFlags;
		layoutBindingFlags.emplace_back(VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT | VK_DESCRIPTOR_BINDING_UPDATE_UNUSED_WHILE_PENDING_BIT);
...
void ImGui_ImplVulkan_UpdateRenderPass(VkRenderPass render_pass)
{
	g_RenderPass = render_pass;
}
...
void ImGui_ImplVulkanH_DestroyFrame(VkDevice device, ImGui_ImplVulkanH_Frame *fd, const VkAllocationCallbacks *allocator)
{
	...
	if (fd->CommandBuffer && fd->CommandPool)
